...
[![brainlife<span>.io</span>/app](https://img.shields.io/badge/brainlife<span>.io</span>-app-green.svg)](https://brainlife<span>.io</span>/app/5f3593e84615e04651bf9364)
...
brainlife<span>.io</span> is publicly funded and for the sustainability of the project it is helpful to acknowledge the use of the platform. We kindly ask that you acknowledge the funding below in your publications and code reusing this code.
...
C-PAC is a BIDS app, so it requires a BIDS directory. This brainlife<span>.io</span> app expects that BIDS directory to be a directory called `bids` in the same directory as the `main` script.
...
#### Simulating brainlife<span>.io</span> locally

1. `git clone` this repo.
2. If you don't have your own Brainlife input file, you can download sample datasets from brainlife<span>.io</span>, or you can use [Brainlife CLI](https://github.com/brain-life/cli) (again, in the cloned directory).
    ```BASH
...
3. Inside the cloned directory, create `config.json` with something like the following content with paths to your input files. Brainlife generates this config file automatically based on your selections in the web GUI.
    ...
    └── README.md
    ```
    which is quite like what the directory structure would be on brainlife<span>.io</span>.
4. Launch the App by executing `main`
    ```bash
    ./main
    ```
...
All of these dependencies are available on brainlife<span>.io</span>.
